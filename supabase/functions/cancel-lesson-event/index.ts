// Edge function: deletes the Google Calendar event for a cancelled lesson.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: lesson, error } = await supabase
      .from("lessons")
      .select("id, google_event_id, student_id")
      .eq("id", lessonId)
      .single();
    ...
    const resp = await googleCalendarFetch(
      accessToken,
      `/calendars/primary/events/${lesson.google_event_id}?sendUpdates=all`,
      { method: "DELETE" },
    );

    if (!resp.ok && resp.status !== 404 && resp.status !== 410) {
      const data = await resp.text();
      console.error("Failed to delete event:", resp.status, data);
      throw new Error(data);
    }

    await supabase.from("lessons").update({ google_event_id: null, meet_link: null }).eq("id", lessonId);
    ...
    console.error("cancel-lesson-event error:", error);
    return new Response(
